...
We know that the PID server runs for 6-7 milliseconds to be able to serve a single request. We also know that the robot invokes it 3 times in one "balancing" cycle, therefore if we have a single robot that invokes the PID server 3 times every 100ms, we need to set the PID server's minimum CPU consumption to somewhere between 18-21ms in 100ms, that translates to 21% of CPU usage. However, if we scale out the robot, we need to increase the PID server's CPU consumption accordingly. This means 36-42% in case of 2 and 54-63% in case of 3 robots, etc.
