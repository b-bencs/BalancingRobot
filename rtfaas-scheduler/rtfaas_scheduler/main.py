...
EDF_MODE = os.environ.get("EDF_MODE", "cpu")
...
def bootstrap_state_from_cluster():
    ...
    for p in pods:
        ...
        if u is None or deadline is None:
            continue
...
def periodic_recheck():
    ...
    for node, u in util_node.items():
        print(f"[RECHECK] node={node} sumU={u:.3f}")
    ...
    if EDF_MODE == "node":
        ...
        for node_name, u in util_node.items():
            ...
            if u > limit + 1e-9:
                print(
                    f"[WARN] node={node_name} node-mode utilization exceeded: sumU={u:.3f} > (m+1)/2={limit:.3f}"
                )
