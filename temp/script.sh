...
show_help() {
    ...
    echo "  --tc <value>               TC type of degradation (delay,jitter,loss)"
    echo "  --tcval <value>           TC degradation value"
    echo "  --help                     Show this help message"
    echo
    echo "Example usage:"
    echo "  ./main_script.sh --timeout 40 --fps 30 --cpu 4 --load 80 --stress-time 2m --host node4 --server-yaml V4/Balancing-.../functions/pidserver_gd.yaml --mode gd --cpu-limit 200m --delay 100ms --jitter 20ms --loss 2%"
}
...
STRESS_TIME="60s"
REMOTE_HOST="node3"
SERVER_YAML_PATH="/users/szefoka/Dec/BalancingRobot/functions/pidserver.yaml"  # Default YAML path
MODE="be"
RUNTIME="5"
DEADLINE="100"
PERIOD="100"
...
# Traffic Control setup (before running the test)
if [ -n "$TC" ] || [ -n "$TC_VAL" ]; then
    echo "Setting up traffic control on $REMOTE_HOST (interface eno1)"
    TC_CMD="sudo tc qdisc add dev eno1 root netem $TC $TC_VAL"
    # Apply the traffic control
    ssh "$REMOTE_HOST" "$TC_CMD"
    echo "Traffic control applied: $TC_CMD"
else
    echo "No traffic control options provided. Skipping TC setup."
fi
...
if [ "$MODE" == "edf" ]; then
    ...
    SERVER_YAML_PATH="/Dec/BalancingRobot/functions/pidserver_gd.yaml"  # GD mode YAML path
...
# Fixed paths
ROBOT_YAML_PATH="/users/szefoka/Dec/BalancingRobot/Robot/deployment_rt.yaml"
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: robot
        image: botondbencs/edfrobotcontainer_2
...
bash /users/szefoka/Dec/BalancingRobot/STRESS/stress_cpu_remote.sh --cpu "$CPU_CORES" --load "$LOAD_LEVEL" --time "$STRESS_TIME" --host "$REMOTE_HOST" &
...
# Remove traffic control after the test is complete
if [ -n "$DELAY" ] || [ -n "$JITTER" ] || [ -n "$LOSS" ]; then
    echo "Removing traffic control on $REMOTE_HOST (interface eno1)"
    ssh "$REMOTE_HOST" "sudo tc qdisc del dev eno1 root netem"
    echo "Traffic control removed."
else
    echo "No traffic control options were applied, skipping removal."
fi

# Stop the stress test
#killall stress_cpu_remote.sh
#echo "Stress test stopped."
# Now collect the results and dump them into a CSV file
CSV_FILE="$LOCAL_DIR/input_parameters.csv"

# Create a CSV file and write the input parameters
echo "timeout,fps,cpu,load,stress-time,host,server-yaml,mode,runtime,deadline,period,cpu-limit,tc,tc-value" > "$CSV_FILE"
echo "$TIMEOUT,$FPS,$CPU_CORES,$LOAD_LEVEL,$STRESS_TIME,$REMOTE_HOST,$SERVER_YAML_PATH,$MODE,$RUNTIME,$DEADLINE,$PERIOD,$CPU_LIMIT,$TC,$TC_VAL" >> "$CSV_FILE"
